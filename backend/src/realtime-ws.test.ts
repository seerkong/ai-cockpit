...
import { createRealtimeWsHandler, tryUpgradeRealtimeWs } from './realtime-ws'
...
function createRegistry(ws: WorkspaceRecord): WorkspaceRegistryLike {
  return {
    ...
    getByToken(token: string) {
      return token === ws.token ? ws : undefined
    },
    disconnect() {
      return false
    },
...
function wsUrl(port: number, path: string) {
  // Bun's WebSocket client uses ws://.
...
describe('realtime ws endpoint', () => {
  test('handshake + subscribe yields snapshot', async () => {
    ...
    const result = await new Promise<{ stateVersion: number; workspaceId: string; sessionIds: string[] }>((resolve, reject) => {
      ...
      client.onopen = () => {
        client.send(JSON.stringify({ type: 'subscribe', payload: { sessionIds: ['sess_123'] } }))
      }
      ...
      client.onmessage = (event) => {
        ...
        resolve({
          ...
          sessionIds: state.subscriptions.sessionIds,
        })
      }
    })
  ...
  test('subscribed clients receive patch updates from upstream events', async () => {
    ...
    // Keep the stream open so patches must flush before end-of-stream.
    ...
    const got = await new Promise<{ patchFrameCount: number; hasPartUpdate: boolean; hasLatestText: boolean }>((resolve, reject) => {
      ...
      client.onmessage = (event) => {
        ...
        hasLatestText = hasLatestText || ops.some((op) => {
          if (op.op !== 'add') return false
          if (op.path !== '/parts/byId/part_1') return false
          if (!isJsonRecord(op.value)) return false
          return op.value.text === 'he'
        })
