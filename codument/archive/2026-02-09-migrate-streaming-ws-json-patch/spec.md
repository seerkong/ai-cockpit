# Spec: WebSocket + JSON Patch Realtime Channel

## Overview

This track migrates `ai-cockpit`'s real-time UI updates (AI streaming output and related session/workspace state) from Server-Sent Events (EventSource) to a WebSocket-based channel.

The WebSocket channel streams RFC 6902 JSON Patch operations against a normalized "realtime state" document, which enables:
- true incremental rendering (no "buffer until done" behavior)
- a single unified realtime channel for messages, sessions, permissions, questions
- future extensibility for complex nested UI cards (A2UI-style interactions)

Key constraints from the user:
- WebSocket is workspace-scoped.
- JSON Patch targets a normalized state tree (maps keyed by stable IDs).
- Reconnect behavior is snapshot-based (no seq resume required).
- Existing SSE endpoint remains as a fallback.
...
- Defining a full A2UI card schema and nested interaction library.
- A full seq-based resume protocol (missing patches are handled by snapshot).
- Using experimental browser APIs (e.g. WebSocketStream) as a hard dependency.
