# /work UI/UX 体验优化 — 规范增量

## 参考文档

- opencode 官方文档：https://open-code.ai/zh/docs/server
- opencode web 源码：`E:\ai-dev\src\opencode\packages\app\src\pages\session\`
- opencode UI 组件：`E:\ai-dev\src\opencode\packages\ui\src\components\`
...
系统 SHALL 在左侧 Connections 面板中，使用 dockview 内置 Paneview（类似 VSCode Debug 侧栏 Variables/Watch/Breakpoints 的纵向可折叠分区）按业务状态将连接分为多个可展开/收起的分栏。

分栏顺序（从上到下）：
1. **Idle** — 空闲状态的连接
2. **Waiting** — 等待人工确认（回答问题、审批权限等）的连接
3. **Active** — 对话进行中的连接
4. **Other** — 其他状态（connecting、error 等）
...
系统 SHALL 将原本显示在 Chat 输入框上方的业务状态标签（如 "Idle"）迁移到左侧 Connections 列表中每个连接条目上显示。

#### Scenario: 状态标签显示在连接条目
- **GIVEN** 用户查看 Connections 面板
- **WHEN** 连接列表渲染
- **THEN** 每个连接条目显示其当前业务状态标签（Idle / Busy / Waiting 等）
...
- **THEN** 底部状态栏显示上下文占用百分比（input tokens / model context limit）
...
系统 SHALL 在 Active 分栏中的每个连接条目上，显示从上一次发起/继续对话到当前的处理时间，便于发现卡住的连接。
...
系统 SHALL 保持 dockview Chat Tab 标题为 "Chat" 不变，在对话区域顶部新增独占一行的会话标题栏，标题栏右侧有"更多"按钮可展开操作菜单。
...
- **THEN** 弹出操作菜单，包含可用操作（如 Fork、Share、Summarize、Revert 等）

#### Scenario: 无会话时标题栏隐藏或显示占位
- **GIVEN** 用户未选中任何会话
- **WHEN** Chat 面板渲染
- **THEN** 标题栏显示 "No session selected" 或隐藏

---

### Requirement: 底部面板整体展开/收起

系统 SHALL 支持 /work 中间区域底部（Terminal/Console 所在部分）整体展开/收起。

#### Scenario: 点击 Toggle 按钮收起底部面板
- **GIVEN** 底部面板处于展开状态
- **WHEN** 用户点击底部面板标题栏的 Toggle 按钮
- **THEN** 底部面板收起（高度缩至 0 或仅显示标题栏）
...
系统 SHALL 将 Chat 上方工具栏中原本独立一行的勾选项（Tools、Reasoning、Expand tools）收纳到工具栏右侧的一个配置按钮中，通过 Dropdown Menu 展开配置。

#### Scenario: 点击配置按钮展开选项
- **GIVEN** Chat 工具栏显示 Fork / Share 等按钮
- **WHEN** 用户点击工具栏右侧的齿轮/配置图标
...
- **THEN** 对话消息区域立即按新配置重新渲染（显示/隐藏工具调用、推理过程等）
...
- **GIVEN** 用户在 Chat 输入框旁
- **WHEN** 用户点击 "+" 按钮
- **THEN** 弹出文件搜索弹窗（调用 /find/file API）
- **AND** 用户可搜索并选择文件路径附加到消息

#### Scenario: 粘贴图片自动附加
- **GIVEN** 用户在 Chat 输入框中
- **WHEN** 用户粘贴剪贴板中的图片
- **THEN** 图片作为 file part（mime: image/*）附加到待发送消息
- **AND** 输入框下方显示图片缩略图预览
...
- **THEN** 弹出 Model 选择弹窗
- **AND** 弹窗顶部有搜索输入框
- **AND** 模型按 provider 分组显示（保持当前分组逻辑）
- **AND** 每个模型显示当前项目的显示名（保持当前显示名逻辑）
...
系统 SHALL 将 Permission 和 Question 确认交互从右侧面板迁移到 Chat 输入框上方的 Dock 浮层中，参考 opencode web 的交互模式。
...
- **WHEN** 当前会话有多个待回答问题
- **THEN** Dock 浮层显示分页导航（progress segments）
...
- **GIVEN** 对话历史中有消息
- **WHEN** 消息渲染
- **THEN** 每条消息显示创建时间（格式：h:mm AM/PM 或 HH:mm）

#### Scenario: 显示已完成消息的耗时
- **GIVEN** assistant 消息已完成（有 time.completed）
- **WHEN** 消息渲染
- **THEN** 显示 turn 耗时（从 user message created 到 assistant completed）
- **AND** 格式为 "Xs" 或 "Xm Ys"

#### Scenario: 进行中消息动态刷新耗时
- **GIVEN** assistant 消息正在进行中（无 time.completed）
- **WHEN** 消息渲染
...
原有的扁平连接列表 SHALL 改为按状态分栏的 Splitview 布局（见 ADDED: 连接列表状态分栏显示）。
