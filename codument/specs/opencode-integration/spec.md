...
This spec defines how `ai-cockpit` integrates with a local OpenCode server for workspace and chat workflows.

Primary goals:
- Support connecting to an already-running local `opencode serve` instance via a configured port.
- Provide a usable model picker (group by provider + keyword search).
- Ensure custom providers/models exposed by OpenCode are visible in the UI.
...
- Treat `providerID` and `modelID` as canonical identifiers end-to-end.
- Avoid redundant `/sessions` refresh storms during prompt lifecycles.
